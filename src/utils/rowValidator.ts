import { UserStoryRow, RowError } from './types';

export function validateUserStoryRows(rows: UserStoryRow[]): {
    validRows: { row: UserStoryRow, originalIndex: number }[],
    errors: RowError[]
} {
    const validRows: { row: UserStoryRow, originalIndex: number }[] = [];
