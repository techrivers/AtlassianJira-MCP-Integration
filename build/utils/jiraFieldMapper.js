...
class JiraFieldMapper {
    ...
    async buildJiraPayload(row, fieldMapping, projectKey) {
        ...
        const issueTypeColumn = Object.keys(row).find(key => key.toLowerCase().includes('issue type') ||
    ...
    async validateFieldPermissions(fieldMapping, projectKey, issueTypeName = 'Story') {
        const validatedMapping = {};
        // Test field permissions by attempting to get create metadata
        try {
            const config = this.getAuthConfig();
            const response = await axios_1.default.get(`${this.baseUrl}/rest/api/3/issue/createmeta?projectKeys=${projectKey}&expand=projects.issuetypes.fields`, config);
            ...
            const availableIssueTypes = project.issuetypes?.map((it) => it.name) || [];
            const targetIssueType = project.issuetypes?.find((it) => it.name === issueTypeName);
            if (!targetIssueType) {
    ...
    async getAvailableIssueTypes(projectKey) {
        try {
            const config = this.getAuthConfig();
            const response = await axios_1.default.get(`${this.baseUrl}/rest/api/3/issue/createmeta?projectKeys=${projectKey}&expand=projects.issuetypes`, config);
            const project = response.data.projects?.find((p) => p.key === projectKey);
            return project?.issuetypes?.map((it) => it.name) || [];
        }
        catch (error) {
