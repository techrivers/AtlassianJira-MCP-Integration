...
exports.sheetToJiraStoriesHandler = sheetToJiraStoriesHandler;
exports.registerSheetToJiraStoriesTool = registerSheetToJiraStoriesTool;
const jiraIssueCreator_1 = require("../utils/jiraIssueCreator");
const jiraFieldMapper_1 = require("../utils/jiraFieldMapper");
const zod_1 = require("zod");
async function sheetToJiraStoriesHandler(input) {
    let rows = [];
    if (input.fileBuffer) {
    ...
    rows.forEach((row, idx) => {
        ...
        const titleField = Object.keys(row).find(key => key.toLowerCase().includes('title') ||
            key.toLowerCase().includes('summary'));
        // Look for description field (case insensitive)
        const descField = Object.keys(row).find(key => key.toLowerCase().includes('description') ||
    ...
    for (const { row, originalIndex } of validRows) {
        const result = await (0, jiraIssueCreator_1.createJiraStory)(row);
        if (result.success) {
            created++;
        }
        else {
...
const sheetToJiraStoriesSchema = zod_1.z.object({
    fileBuffer: zod_1.z.string().optional().describe("Excel file buffer (base64, optional)"),
    googleSheetLink: zod_1.z.string().optional().describe("Public Google Sheets link (optional)")
});
function registerSheetToJiraStoriesTool(server) {
    server.tool("sheetToJiraStories", "Creates Jira stories from a spreadsheet (Excel or Google Sheets link).", sheetToJiraStoriesSchema.shape, async (input) => {
