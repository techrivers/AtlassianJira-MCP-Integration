...
exports.registerMeetingNotesToJiraTool = registerMeetingNotesToJiraTool;
...
function determineIssueType(text, meetingType) {
    if (patterns.bugReport.test(text))
        return 'Bug';
    if (patterns.userStory.test(text))
        return 'Story';
    return meetingConfigs[meetingType].defaultIssueType;
}
function estimateStoryPoints(text) {
    const complexity = text.toLowerCase();
    if (/(?:simple|fix|update|change|quick)/i.test(complexity))
        return 1;
    if (/(?:implement|create|add|build)/i.test(complexity))
        return 3;
    if (/(?:redesign|integrate|migrate|complex)/i.test(complexity))
        return 8;
    return undefined;
}
function parseActions(text, meetingType, defaultProject, defaultAssignee) {
    const actions = [];
...
function createAction(text, meetingType, defaultAssignee) {
...
async function createJiraIssue(action, project, fieldMapper, validatedMapping) {
    ...
    if (action.assignee)
        mockRow['Assignee'] = action.assignee;
    if (action.storyPoints)
        mockRow['Story Point Estimate'] = action.storyPoints;
...
const meetingNotesToJiraSchema = zod_1.z.object({
    meetingNotes: zod_1.z.string().optional().describe("Raw meeting notes text"),
    fileBuffer: zod_1.z.instanceof(Buffer).optional().describe("Text file buffer (.txt, .md, .rtf)"),
    filename: zod_1.z.string().optional().describe("Original filename (required when using fileBuffer)"),
    meetingType: zod_1.z.enum(["standup", "planning", "retrospective", "general"]).describe("Type of meeting for context-aware parsing"),
    defaultProject: zod_1.z.string().describe("Project key for created issues (e.g., 'PROJ')"),
    defaultAssignee: zod_1.z.string().optional().describe("Default assignee email for unassigned items"),
    autoCreateEpic: zod_1.z.boolean().optional().default(false).describe("Create epic if multiple related stories found"),
    dryRun: zod_1.z.boolean().optional().default(false).describe("Parse and show results without creating issues")
});
function registerMeetingNotesToJiraTool(server) {
    // @ts-expect-error: server.tool signature is not typed
    server.tool("meetingNotesToJira", "Parse meeting notes and create Jira issues from action items. Supports both direct text input and file upload (.txt, .md, .rtf).", meetingNotesToJiraSchema.shape, async (input) => {
        const config = configManager_1.dynamicConfig.getConfig();
        ...
        if (dryRun) {
            const preview = actions.map((action, index) => `${index + 1}. [${action.issueType}] ${action.summary}\n   Assignee: ${action.assignee || 'Unassigned'}\n   Priority: ${action.priority}\n   Labels: ${action.labels.join(', ')}`).join('\n\n');
            return {
                content: [{
                        type: "text",
