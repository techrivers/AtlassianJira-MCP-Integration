...
exports.runSecureCLIConfiguration = runSecureCLIConfiguration;
...
class SecureJiraConfigurationCLI {
    ...
    async validateJiraConnection(credentials) {
        ...
        try {
            ...
            const response = await axios_1.default.get(`${credentials.url}/rest/api/2/myself`, {
                headers: {
            ...
            if (response.status === 200) {
                ...
                if (credentials.projectKey) {
                    try {
                        await axios_1.default.get(`${credentials.url}/rest/api/2/project/${credentials.projectKey}`, {
                            headers: {
                                'Authorization': `Basic ${auth}`,
                                'Accept': 'application/json'
                            },
    ...
    async getSecureInput(prompt, validator, isSecret = false) {
        let attempts = 0;
        const maxAttempts = 3;
    ...
    async tryOSCredentialStorage(credentials) {
    ...
    storeEncryptedCredentials(credentials) {
        try {
            ...
            fs_1.default.writeFileSync(configPath, JSON.stringify(config, null, 2), { mode: 0o600 });
    ...
    async run() {
        try {
            ...
            console.log();
            const url = await this.getSecureInput('🔗 Jira URL: ', this.validateJiraUrl);
            // Step 2: Get Username
            this.printStep(2, 'Username / Email');
            console.log('Enter the email address you use to log into Jira');
            console.log();
            const username = await this.getSecureInput('👤 Username/Email: ', this.validateEmail);
            ...
            console.log();
            const apiToken = await this.getSecureInput('🔑 API Token (hidden): ', this.validateApiToken, true);
...
async function runSecureCLIConfiguration() {
